...
class Utils:
    ...
    # def mynormal(self,component, key):
    #     if key == 'penalty_main':
    #         weight_min = 1000
    #         weight_max = 2000
    #     elif key == 'penalty_win':
    #         weight_min = 100
    #         weight_max = 200
    #     elif key == 'cost':
    #         weight_min = 0
    ...
    #
    #     if M - m == 0:
    #         return weight_min * np.ones(len(component))
    #     else:
    #         return weight_min + (
    #                     ((component - m) * (weight_max - weight_min)) / (M - m))  # normalise between [a,weight]
    ...
    def greedy_selection_constrained(self,fitness):
        if self.problem.greedy_alpha == 0:
            cost = np.array([x[0] for x in fitness])
            penalty_main = [x[1] for x in fitness]
            r = np.lexsort((cost, penalty_main))
            winner_idx = r[0]
        else:
            'FIND CLUSTER'
            Feasible = []
    ...
    def valid_local(self, segment, local_solution, i, j):
        for edge in segment:
            idx = edge[0]
            for index in [idx-1,idx]:
                # check two pairs of edge
                cut = local_solution.visited[index][0]
                lst = cut.prec['SN']
                if len(lst) > 0:
                    before = set(local_solution.reclaimed_cuts_keys[:idx-1])
