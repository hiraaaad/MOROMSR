from core_aco_case3.algorithm import np, pd
from core_aco_case3.node import Solution
...
class Ant():
    ...
    def __repr__(self):
        return f'Ant(avg_penalty={self.solution.viol_main}, window_penalty={self.solution.viol_window}, obj={self.solution.obj})'
    ...
    def construct_solution_case_3(self):
        self.reclaim_initial('01-01-01-01-01')
        for demand in range(self.problem.demand):
            self.termination = False
            while self.termination == False:
                ...
                next_idx = self.greedy_selection_constrained_case3(fitness_candidates, ph_info)
                next_node = neighbor_lst[next_idx][0]
                next_node_direction = neighbor_lst[next_idx][1]
                self.solution.obj += fitness_candidates[next_idx][0]  # cost
                self.solution.parcel_list[-1].penalty_mineral_avg = fitness_candidates[next_idx][2]  # average_limits
    ...
    def calc_penalty_window(self, neighbor_info):
        if self.solution.parcel_list[-1].length > 2:
            ...
            (penalty_win_lower, penalty_win_upper) = self.calc_penalty_window_ul(window_avg)
    ...
    def greedy_selection_constrained_case3(self, fitness, ph_info):
        'FIND CLUSTER'
        Feasible = []
        SemiFeasible = []
        InFeasible = []
    ...
    def local_search(self, initial_solution):
        ...
        for i in range(1, len(initial_visited)):
            for j in range(1, len(initial_visited)):
                if i != j:
                    if (self.problem.ls in [1, 3] and j > i) or self.problem.ls == 2:
                        ...
                        if condition is True:
                            ...
                            if self.valid_local(L, local_solution, i, j):
                                # print(local_solution.visited[i],local_solution.visited[j],i,j)
